...
*/
static ssize_t cameralog_read(struct file *fp, char __user *buff, size_t length,
			      loff_t *ppos)
{
	...
	if (strlen(camlog_dev->m_camlog_message)) {
		uint8_t camlog_message[1024];
		strcpy(camlog_message, camlog_dev->m_camlog_message);
	...
	memset(camlog_dev->m_camlog_message, 0,
	       strlen(camlog_dev->m_camlog_message));
	return bytes_read;
}
...
static ssize_t cameralog_write(struct file *fp, const char *buff, size_t length,
			       loff_t *ppos)
{
	int ret = 0;
	ssize_t bytes_read = 0;
	struct camlog_dev *camlog_dev = &camlogdev;
	mutex_lock(&camlog_dev->camlog_message_lock);
	ret = copy_from_user(camlog_dev->m_camlog_message, buff, length);
	if (0 == ret) {
		bytes_read = strlen(camlog_dev->m_camlog_message);
	}
...
static int __init cameralog_init(void)
{
	...
	pr_err("liuhang debug Failed to create class.\n");
	if (IS_ERR(camlog_class)) {
		unregister_chrdev(camlog_major, CAMLOG_DEV_NAME);
		pr_warn("Failed to create class.\n");
		return PTR_ERR(camlog_class);
	}
	pr_err("liuhang debug1 Failed to create class.\n");
	camlog_dev->devt = MKDEV(camlog_major, 0);
	device = device_create(camlog_class, NULL, camlog_dev->devt, NULL,
			       CAMLOG_DEV_NAME);
	if (IS_ERR(device)) {
