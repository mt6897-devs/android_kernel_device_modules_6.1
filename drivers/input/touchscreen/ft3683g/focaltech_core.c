...
int fts_get_lockdown_information(struct fts_ts_data *ts_data)
{
	...
	for (i = 0; i < 8; i++) {
		count += sprintf(temp_lockdown + count, " %02x ",
				 ts_data->lockdown_info[i]);
	}
	FTS_INFO("lockdown information before formatting:%s",
	...
	for (i = 0; i < ts_data->touch_event_num; i++) {
		...
		if (EVENT_DOWN(events[i].flag)) {
			...
			if ((ts_data->log_level >= 3) ||
				...
				__test_and_set_bit(events[i].id,
						   &fts_data->touch_id);
				...
				if (events[i].id == 0) { //first finger down
					...
					if (touch_static) {
						down_time = ktime_get();
						FTS_INFO(
							"touch coord static down");
						if (touch_boost_en) {
							FTS_INFO(
								"notify hal to boost");
							add_common_data_to_buf(
								0,
								SET_CUR_VALUE,
								Touch_GAMETURBOTOOL_FOLLOWUP,
								1,
								&touch_boost);
	...
	if (unlikely(touch_point_pre ^ touch_down_point_cur)) {
		for (i = 0; i < max_touch_num; i++) {
			if ((1 << i) &
				...
				__test_and_clear_bit(events[i].id,
						     &fts_data->touch_id);
	...
	if (touch_down_point_cur)
		...
		if (ts_data->touch_points && (ts_data->log_level >= 1)) {
			...
			if (touch_static) {
				...
				if (ktime_to_ms(ktime_sub(up_time, down_time)) >
				    20) {
					coord_cnt++;
		...
		if (buf[8] == 0) {
			...
			if (!data->suspended) {
				if (!data->fod_status) {
					...
					FTS_INFO(
						"Turn off FOD enable in the CF register successfully！\n");
				}
				FTS_INFO(
					"FTS:touch is not in suspend state, report x,y value by touch nomal report\n");
				return -EINVAL;
...
static int fts_pinctrl_init(struct fts_ts_data *ts)
{
	...
	ts->pinctrl_state_spimode =
		pinctrl_lookup_state(ts->pinctrl, "pmx_gt_spi_mode");
	...
	if (enable) {
		if (ts_data->power_disabled) {
			...
			if (!IS_ERR_OR_NULL(ts_data->avdd)) {
				...
				if (ret) {
					FTS_ERROR(
						"enable avdd regulator failed,ret=%d",
						ret);
				}
			}
			if (ts_data->pdata->avdd_gpio > 0) {
				gpio_direction_output(ts_data->pdata->avdd_gpio,
						      1);
...
static int fts_power_source_init(struct fts_ts_data *ts_data)
{
	...
	if (strlen(ts_data->pdata->avdd_name)) {
		ts_data->avdd =
			regulator_get(ts_data->dev, ts_data->pdata->avdd_name);
		if (IS_ERR_OR_NULL(ts_data->avdd)) {
		...
		if (regulator_count_voltages(ts_data->avdd) > 0) {
			ret = regulator_set_voltage(
				ts_data->avdd, FTS_VTG_MIN_UV, FTS_VTG_MAX_UV);
			if (ret) {
				FTS_ERROR(
					"avdd regulator set_vtg failed ret=%d",
					ret);
	...
	if (strlen(ts_data->pdata->iovdd_name)) {
		ts_data->iovdd =
			regulator_get(ts_data->dev, ts_data->pdata->iovdd_name);
		if (IS_ERR_OR_NULL(ts_data->iovdd)) {
...
{
	struct fts_ts_data *ts_data =
		container_of(work, struct fts_ts_data, sleep2gesture_work);
...
static int fts_set_cur_value(int mode, int value)
{
	...
	if (mode == Touch_Report_Rate) {
		FTS_INFO("Mode:Touch_Report_Rate  Report_Rate_status = %d",
			 value);
		if (value == 0) {
			...
			if (ret < 0) {
				FTS_ERROR(
					"Failed to switch Report_Rate to 120HZ, ret=%d",
					ret);
				return -EINVAL;
			}
		}
		if (value == 1) {
			...
			if (ret < 0) {
				FTS_ERROR(
					"Failed to switch Report_Rate to 240HZ, ret=%d",
					ret);
				return -EINVAL;
			}
		}
	...
	if (mode == Touch_Fod_Enable && value >= 0) {
		...
		if (fts_data->fod_status == 1) {
			...
			if (state != 0x03) {
				...
				if (state == 0x03)
					queue_work(
						fts_data->ts_workqueue,
						&fts_data->sleep2gesture_work);
				else
					fts_fod_recovery();
			}
		}
...
static int fts_touch_doze_analysis(int input)
{
	...
	case DISABLE_IRQ:
		...
		break;
	case REGISTER_IRQ:
		error = request_threaded_irq(fts_data->irq, NULL,
					     fts_irq_handler,
					     fts_data->pdata->irq_gpio_flags,
					     FTS_DRIVER_NAME, fts_data);
		FTS_INFO("irq:%d, flag:%x", fts_data->irq,
			 fts_data->pdata->irq_gpio_flags);
		if (error < 0)
	...
	case IRQ_PIN_LEVEL:
		irq_status =
			gpio_get_value(fts_data->pdata->irq_gpio) == 0 ? 0 : 1;
		break;
	case ENTER_SUSPEND:
		flush_workqueue(fts_data->ts_workqueue);
		queue_work(fts_data->ts_workqueue, &fts_data->suspend_work);
...
static void fts_game_idle_high_refresh_recovery(struct fts_ts_data *ts_data)
{
	if (ts_data->gamemode_enabled) {
		FTS_DEBUG(
			"%s, gamemode enabled, recover game_idle_high_refresh\n",
			__func__);
		fts_write_reg(0x8E, true);
	}
}
...
static void fts_fod_status_recovery(struct fts_ts_data *ts_data)
{
	if (ts_data->fod_status != -1 && ts_data->fod_status != 0) {
		FTS_DEBUG("%s, fod_status = %d, enable CF register\n", __func__,
			  ts_data->fod_status);
		if (ts_data->suspended) {
			FTS_DEBUG("%s, tp is in suspend mode, write 0xD0 to 1",
				  __func__);
...
static int fts_palm_sensor_write(int value)
{
	...
	if (!fts_data->palm_sensor_switch) {
		FTS_INFO(
			"disable palm reg and update palm_status_value for palm_sensor node :%d",
			0);
		update_palm_sensor_value(0);
	}
	return ret;
...
static int fts_ts_suspend(struct device *dev)
{
	...
	if ((ts_data->fod_status != 0 && ts_data->fod_status != -1) ||
	    ts_data->doubletap_status || ts_data->aod_status) {
		ts_data->gesture_support = 1;
	}
	/*Fod_status is 0 when the fingerprint is closed after locking the screen*/
	if (!ts_data->doubletap_status && !ts_data->aod_status &&
	    !ts_data->fod_status) {
