...
#endif
#include "include/panel_n11a_41_02_0b_alpha_data.h"
#include "include/panel-n11a-41-02-0b-dsc-vdo.h"
...
static int lcm_unprepare(struct drm_panel *panel)
{
	...
	if (time == 1) {
		...
		push_table(ctx, lcm_suspend_setting2,
			   sizeof(lcm_suspend_setting2) /
				   sizeof(struct LCM_setting_table));
	}
	++time;
	if (time == 3)
...
static void lcm_panel_init(struct lcm *ctx)
{
	...
	push_table(ctx, init_setting_vdo,
...
static int lcm_panel_poweroff(struct drm_panel *panel)
{
	...
	ctx->reset_gpio =
		devm_gpiod_get_index(ctx->dev, "reset", 0, GPIOD_OUT_HIGH);
	if (IS_ERR(ctx->reset_gpio)) {
	...
	//DVDD 1.2V -> 0
	ctx->dvdd_gpio =
		devm_gpiod_get_index(ctx->dev, "dvdd", 0, GPIOD_OUT_HIGH);
...
static struct mtk_panel_params ext_params_30hz = {
	...
	.dyn_fps = {
		...
		.dfps_cmd_table[1] = {0, 2, {0xBE,0x00} },
...
static struct mtk_panel_params ext_params_60hz = {
	...
	.dyn_fps = {
		...
		.short_dfps_cmds_start_index = 3,
		.long_dfps_cmds_counts = 4,
		.dfps_cmd_table[0] = {0, 1, {0x38} },
		.dfps_cmd_table[1] = {0, 6, {0xF0,0x55,0xAA,0x52,0x08,0x08} },
		.dfps_cmd_table[2] = {0, 2, {0xBE,0x03} },
		.dfps_cmd_table[3] = {0, 2, {0x2F,0x02} },
	},
	.physical_width_um = PHYSICAL_WIDTH,
	.physical_height_um = PHYSICAL_HEIGHT,
	.prefetch_time = PREFETCH_TIME,
...
static struct mtk_panel_params ext_params_90hz = {
	...
	.dyn_fps = {
		...
		.dfps_cmd_table[1] = {0, 6, {0xF0,0x55,0xAA,0x52,0x08,0x08} },
		.dfps_cmd_table[2] = {0, 2, {0xBE,0x03} },
		.dfps_cmd_table[3] = {0, 2, {0x2F,0x01} },
	},
	.physical_width_um = PHYSICAL_WIDTH,
	.physical_height_um = PHYSICAL_HEIGHT,
	.prefetch_time = PREFETCH_TIME,
...
static struct mtk_panel_params ext_params_120hz = {
	...
	.dyn_fps = {
		...
		.dfps_cmd_table[1] = {0, 6, {0xF0,0x55,0xAA,0x52,0x08,0x08} },
		.dfps_cmd_table[2] = {0, 2, {0xBE,0x03} },
		.dfps_cmd_table[3] = {0, 2, {0x2F,0x00} },
	},
	.physical_width_um = PHYSICAL_WIDTH,
	.physical_height_um = PHYSICAL_HEIGHT,
	.prefetch_time = PREFETCH_TIME,
				   ...
				   unsigned int mode)
{
	...
	pr_info("%s drm_mode_vrefresh = %d, m->hdisplay = %d\n", __func__,
		drm_mode_vrefresh(m_dst), m_dst->hdisplay);
...
{
	char gir_off_set[] = { 0x5F, 0x01, 0x05 };
	char gir_on_set[] = { 0x5F, 0x01 };
	struct lcm *ctx = NULL;
	...
	if (bl_level > PEAK_HDR_BL_LEVEL && bl_level <= PEAK_MAX_BL_LEVEL &&
	    !ctx->peak_hdr_status) {
	...
	if (bl_level != 0 && bl_level <= PEAK_HDR_BL_LEVEL &&
	    ctx->peak_hdr_status) {
	...
	pr_info("%s: peak_hdr_status = %d ctx->gir_status = %d\n", __func__,
		ctx->peak_hdr_status, ctx->gir_status);
err:
	return 1;
}
	...
	if (stage == BEFORE_DSI_POWERDOWN) {
		if (ctx->gir_status)
			push_table(ctx, mode_120hz_setting_gir_on,
				   sizeof(mode_120hz_setting_gir_on) /
					   sizeof(struct LCM_setting_table));
		else
			push_table(ctx, mode_120hz_setting_gir_off,
				   sizeof(mode_120hz_setting_gir_off) /
					   sizeof(struct LCM_setting_table));
		ctx->dynamic_fps = 120;
	}
	mutex_unlock(&ctx->panel_lock);
	...
	if (stage == BEFORE_DSI_POWERDOWN) {
		if (ctx->gir_status)
			push_table(ctx, mode_90hz_setting_gir_on,
				   sizeof(mode_90hz_setting_gir_on) /
					   sizeof(struct LCM_setting_table));
		else
			push_table(ctx, mode_90hz_setting_gir_off,
				   sizeof(mode_90hz_setting_gir_off) /
					   sizeof(struct LCM_setting_table));
		ctx->dynamic_fps = 90;
	}
	mutex_unlock(&ctx->panel_lock);
	...
	if (stage == BEFORE_DSI_POWERDOWN) {
		if (ctx->gir_status)
			push_table(ctx, mode_60hz_setting_gir_on,
				   sizeof(mode_60hz_setting_gir_on) /
					   sizeof(struct LCM_setting_table));
		else
			push_table(ctx, mode_60hz_setting_gir_off,
				   sizeof(mode_60hz_setting_gir_off) /
					   sizeof(struct LCM_setting_table));
		ctx->dynamic_fps = 60;
	}
	mutex_unlock(&ctx->panel_lock);
		       ...
		       enum MTK_PANEL_MODE_SWITCH_STAGE stage)
{
	...
	isFpsChange = drm_mode_vrefresh(m_dst) == drm_mode_vrefresh(m_cur) ?
			      false :
			      true;

	pr_info("%s isFpsChange = %d, dst_mode vrefresh = %d, cur_mode vrefresh = %d,, vdisplay = %d, hdisplay = %d\n",
		__func__, isFpsChange, drm_mode_vrefresh(m_dst),
		drm_mode_vrefresh(m_cur), m_dst->vdisplay, m_dst->hdisplay);
...
static int panel_doze_disable(struct drm_panel *panel, void *dsi_drv,
			      dcs_write_gce cb, void *handle)
{
	atomic_set(&doze_enable, 0);
	pr_info("%s -\n", __func__);
				     ...
				     int doze_brightness)
{
	...
	if (DOZE_BRIGHTNESS_LBM == doze_brightness ||
	    DOZE_BRIGHTNESS_HBM == doze_brightness) {
		if (DOZE_BRIGHTNESS_LBM == doze_brightness)
			ret = mi_disp_panel_ddic_send_cmd(
				lcm_aod_low_mode, ARRAY_SIZE(lcm_aod_low_mode),
				format);
		else if (DOZE_BRIGHTNESS_HBM == doze_brightness)
			ret = mi_disp_panel_ddic_send_cmd(
				lcm_aod_high_mode,
				ARRAY_SIZE(lcm_aod_high_mode), format);
		atomic_set(&doze_enable, 1);
	}

	if (DOZE_TO_NORMAL == doze_brightness) {
		ret = mi_disp_panel_ddic_send_cmd(bl_tb0, ARRAY_SIZE(bl_tb0),
						  format);
		atomic_set(&doze_enable, 0);
	}
...
static void lcm_esd_restore_backlight(struct drm_panel *panel)
{
	...
	mi_dsi_panel_count_enter(panel, PANEL_BACKLIGHT, last_non_zero_bl_level,
				 0);
#endif
	return;
}
...
static int panel_set_gir_on(struct drm_panel *panel)
{
	...
	if (!ctx->enabled) {
		...
		mi_disp_panel_ddic_send_cmd(gir_on_set, ARRAY_SIZE(gir_on_set),
					    format);
	}
...
static int panel_set_gir_off(struct drm_panel *panel)
{
	...
	if (!ctx->enabled) {
		...
		mi_disp_panel_ddic_send_cmd(gir_off_set,
					    ARRAY_SIZE(gir_off_set), format);
	}
...
static int panel_hbm_fod_control(struct drm_panel *panel, bool en)
{
	...
	if (en) {
		ret = mi_disp_panel_ddic_send_cmd(
			hbm_fod_on_cmd, ARRAY_SIZE(hbm_fod_on_cmd), format);
	} else {
		ret = mi_disp_panel_ddic_send_cmd(
			hbm_fod_off_cmd, ARRAY_SIZE(hbm_fod_on_cmd), format);
	}
	if (ret != 0) {
...
static void mi_parse_cmdline_perBL(struct LHBM_WHITEBUF *lhbm_whitebuf)
{
	...
	if (!lhbm_w1200_update_flag && !lhbm_w200_update_flag &&
	    !lhbm_g500_update_flag) {
...
static int mi_disp_panel_update_lhbm_A9reg(struct mtk_dsi *dsi,
					   enum lhbm_cmd_type type,
					   int flat_mode, int bl_level)
{
	u8 alpha_buf[2] = { 0 };
	...
	if (!lhbm_w1200_readbackdone || !lhbm_w200_readbackdone ||
	...
	case TYPE_WHITE_1200:
		if (flat_mode) {
			...
			lhbm_normal_white_1200nit[0].para_list[12] =
				alpha_buf[0];
			lhbm_normal_white_1200nit[0].para_list[13] =
				alpha_buf[1];
		} else {
	...
	case TYPE_WHITE_200:
		if (flat_mode) {
			...
			pr_info("[%d] alpha_buf[0] = %02hhx, alpha_buf[1] = %02hhx\n",
				type, alpha_buf[0], alpha_buf[1]);
			lhbm_normal_white_200nit[0].para_list[12] =
				alpha_buf[0];
			lhbm_normal_white_200nit[0].para_list[13] =
				alpha_buf[1];
		} else {
			alpha_buf[0] = (giroff_alpha_set[bl_level] >> 8) & 0xff;
			alpha_buf[1] = giroff_alpha_set[bl_level] & 0xff;
	...
	case TYPE_GREEN_500:
		if (flat_mode) {
			...
			pr_info("[%d] alpha_buf[0] = %02hhx, alpha_buf[1] = %02hhx\n",
				type, alpha_buf[0], alpha_buf[1]);
			lhbm_normal_green_500nit[0].para_list[12] =
				alpha_buf[0];
			lhbm_normal_green_500nit[0].para_list[13] =
				alpha_buf[1];
		} else {
			alpha_buf[0] = (giroff_alpha_set[bl_level] >> 8) & 0xff;
			alpha_buf[1] = giroff_alpha_set[bl_level] & 0xff;
...
static int panel_set_lhbm_fod(struct mtk_dsi *dsi,
			      enum local_hbm_state lhbm_state)
{
	struct lcm *ctx = NULL;
	struct mi_dsi_panel_cfg *mi_cfg = NULL;
	int bl_level;
	...
	case LOCAL_HBM_OFF_TO_NORMAL_BACKLIGHT:
		...
		mi_disp_panel_ddic_send_cmd(lhbm_off, ARRAY_SIZE(lhbm_off),
					    format);
		ctx->lhbm_en = false;
		break;
	...
	case LOCAL_HBM_NORMAL_WHITE_110NIT:
		...
		if (atomic_read(&doze_enable)) {
			...
			else if (ctx->doze_brightness_state ==
				 DOZE_BRIGHTNESS_LBM)
				mi_cfg->last_no_zero_bl_level = bl_level_doze;
			mutex_lock(&dsi->dsi_lock);
			mi_disp_panel_update_lhbm_A9reg(dsi, TYPE_WHITE_200,
							flat_mode,
							doze_lbm_dbv_level);
		} else {
			mi_disp_panel_update_lhbm_A9reg(dsi, TYPE_WHITE_200,
		...
		format = FORMAT_LP_MODE | FORMAT_BLOCK;
		mi_disp_panel_ddic_send_cmd(
			lhbm_normal_white_200nit,
			ARRAY_SIZE(lhbm_normal_white_200nit), format);
		ctx->lhbm_en = true;
		break;
	case LOCAL_HBM_NORMAL_GREEN_500NIT:
		...
		mi_disp_panel_update_lhbm_A9reg(dsi, TYPE_GREEN_500, flat_mode,
						bl_level);

		format = FORMAT_LP_MODE | FORMAT_BLOCK;
		mi_disp_panel_ddic_send_cmd(
			lhbm_normal_green_500nit,
			ARRAY_SIZE(lhbm_normal_green_500nit), format);
		ctx->lhbm_en = true;
		break;
	case LOCAL_HBM_NORMAL_WHITE_1000NIT:
		...
		if (atomic_read(&doze_enable)) {
			...
			if (ctx->doze_brightness_state == DOZE_BRIGHTNESS_HBM)
				...
				mi_cfg->last_no_zero_bl_level = bl_level_doze;
			mutex_lock(&dsi->dsi_lock);
			mi_disp_panel_update_lhbm_A9reg(dsi, TYPE_WHITE_1200,
							flat_mode,
							doze_lbm_dbv_level);
		} else {
			mi_disp_panel_update_lhbm_A9reg(dsi, TYPE_WHITE_1200,
		...
		format = FORMAT_LP_MODE | FORMAT_BLOCK;
		mi_disp_panel_ddic_send_cmd(
			lhbm_normal_white_1200nit,
			ARRAY_SIZE(lhbm_normal_white_1200nit), format);
	...
	case LOCAL_HBM_HLPM_WHITE_110NIT:
		...
		mi_disp_panel_update_lhbm_A9reg(dsi, TYPE_WHITE_200, flat_mode,
						bl_level_doze);
		format = FORMAT_LP_MODE | FORMAT_BLOCK;
		mi_disp_panel_ddic_send_cmd(
			lhbm_normal_white_200nit,
	...
	case LOCAL_HBM_HLPM_WHITE_1000NIT:
		...
		mi_disp_panel_update_lhbm_A9reg(dsi, TYPE_WHITE_1200, flat_mode,
						bl_level_doze);
...
{
	pr_info("%s: panel=%p, level=%d\n", __func__, panel, level);
	return 0;
}
#endif
...
static int lcm_probe(struct mipi_dsi_device *dsi)
{
	...
	ext_params_60hz.err_flag_irq_gpio = of_get_named_gpio_flags(
		...
		(enum of_gpio_flags *)&(ext_params_60hz.err_flag_irq_flags));
#ifdef ENABLE_30HZ
	...
	ext_params_120hz.err_flag_irq_flags =
		ext_params_60hz.err_flag_irq_flags;
#endif
	...
	pr_info("%s n11a-41-02-0b-dsc-vdo -\n", __func__);
	return ret;
}
...
static const struct of_device_id lcm_of_match[] = {
	{
		.compatible = "n11a_41_02_0b_dsc_vdo,lcm",
	},
	{}
};
...
static struct mipi_dsi_driver lcm_driver = {
	...
	.remove = lcm_remove,
	.driver = {
		.name = "n11a_41_02_0b_dsc_vdo,lcm",
		.owner = THIS_MODULE,
...
module_param_string(oled_lhbm, oled_lhbm_cmdline, sizeof(oled_lhbm_cmdline),
		    0600);
MODULE_PARM_DESC(oled_lhbm, "oled_lhbm=<local_hbm_info>");

MODULE_AUTHOR("rio004 <riyon336@gmail.com>");
MODULE_DESCRIPTION("n11a_41_02_0b_dsc_vdo oled panel driver");
