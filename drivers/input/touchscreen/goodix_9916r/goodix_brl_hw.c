...
static int brl_dev_confirm(struct goodix_ts_core *cd)
{
	...
	while (retry--) {
		ret = hw_ops->write(cd, BOOTOPTION_ADDR, tx_buf, sizeof(tx_buf));
	...
	u8 target_status, int retry)
{
	...
	for (i = 0; i < retry; i++) {
		ts_debug("cmd ack before  is %*ph", (int)sizeof(cmd_ack),cmd_ack.buf);
	...
	struct goodix_ic_info *ic_info)
{
	...
	if (!ic_info->misc.cmd_addr || !ic_info->misc.fw_buffer_addr ||
	    !ic_info->misc.touch_data_addr || !ic_info->misc.self_diffdata_addr) {
				...
				u8 *pre_buf, u32 pre_buf_len)
{
	...
	if (debug_buffer[1] & 0x80){
    	ts_debug("touch_debug_info %*ph", 12, debug_buffer);/*目前看debug信息输出太多，后续固件有优化再按需求改成ts_info*/
	}
			 ...
			 struct goodix_ts_event *ts_event)
{
	...
	if (checksum_cmp(pre_buf, IRQ_EVENT_HEAD_LEN, CHECKSUM_MODE_U8_LE)) {
		...
		if ( (pre_buf[4] == 0x55) && (pre_buf[0] == 0) && (pre_buf[2] == 0x0a) &&
			(pre_buf[3] == 0x01) && (cd->work_status == TP_GESTURE) ) {
			ts_info("warning: fod up checksum err");
			/*walkaround for avoid drop fod up*/
			pre_buf[0] = 0x28;
		}
		else if ( (pre_buf[4] == 0x46) && (pre_buf[0] == 0) && (pre_buf[2] == 0x0a) &&
			(pre_buf[3] == 0x01) && (cd->work_status == TP_GESTURE ) ) {
		...
		struct ts_rawdata_info *info)
{
	...
	if (checksum_cmp(frame_buf_data, cd->ic_info.misc.frame_data_head_len, CHECKSUM_MODE_U8_LE)) {
...
static int brl_send_Edge_data(struct goodix_ts_core *cd)
{
    ...
    */
	...
	ret = hw_ops->write(cd, misc->self_diffdata_addr, cd->edge_data.buf, len);
	if (ret) {
...
int brl_game(struct goodix_ts_core *cd, u8 data0, u8 data1)
{
	...
	if (cd->gamemode_enabled){
		...
		cmd.data[1] = data1;
		if (cd->hw_ops->send_cmd(cd, &cmd)) {
			ts_err("failed send game cmd, data0 = 0x%x, data1 = 0x%x", data0, data1);
			mutex_unlock(&cd->edge_data_mutex);
			return -EINVAL;
		}else {
			ts_info("game data0:0x%x, data1:0x%x, gamemode_enabled = %d", data0, data1, cd->gamemode_enabled);
			ret = wait_cmd_status(cd, CONFIG_CMD_STATUS_PASS,12);
			if(!ret){
					while (retry++ < GOODIX_CMD_RETRY) {
						ret = brl_send_Edge_data(cd);
					...
					cmd.data[0] = 1;
					if (cd->hw_ops->send_cmd(cd, &cmd)) {
						ts_err("failed send game idle cmd, data0 = %d", cmd.data[0]);
						mutex_unlock(&cd->edge_data_mutex);
						return -EINVAL;
					}
					else
