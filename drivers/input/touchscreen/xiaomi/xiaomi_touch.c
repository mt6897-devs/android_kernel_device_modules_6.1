...
static int touch_drm_state_change_callback(struct notifier_block *self,
...
static struct notifier_block touch_drm_state_change_block = {
	.notifier_call = touch_drm_state_change_callback,
};
...
static void register_panel_handler(struct work_struct *work)
{
	...
	ret = mi_disp_register_client(&touch_drm_state_change_block);
	if (ret < 0)
...
{
	mi_disp_unregister_client(&touch_drm_state_change_block);
