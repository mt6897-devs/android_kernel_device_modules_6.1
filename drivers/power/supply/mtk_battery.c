...
{
	if (gm != NULL) {
		gm->project_name = DUCHAMP;

		bm_info("%s project_name = %d\n",
			 __func__, gm->project_name);
        }
}
	...
	union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_MODEL_NAME:
		switch (gm->project_name)
		{
			case DUCHAMP:
				val->strval = "N11A_5500mah_90w";
				break;
			default:
...
static int
moisture_detection_status_get(struct mtk_battery *gm,
			      struct mtk_battery_sysfs_field_info *attr,
			      int *val)
{
	if (gm->info != NULL) {
...
static void set_error(struct mtk_battery *gm)
{
	...
	bm_debug("%s en_ret=%d\n", __func__,
		 gm->smart_chg[SMART_CHG_STATUS_FLAG].en_ret);
}
...
static void handle_smart_chg_functype(struct mtk_battery *gm,
				      const int func_type, const int en_ret,
				      const int func_val)
{
	switch (func_type) {
...
static int handle_smart_chg_functype_status(struct mtk_battery *gm)
{
	...
	all_func_status |=
		!!gm->smart_chg[SMART_CHG_STATUS_FLAG].en_ret; //handle bit0
	...
	for (i = SMART_CHG_FEATURE_MIN_NUM; i <= SMART_CHG_FEATURE_MAX_NUM;
	     i++) { //handle bit1 ~ bit SMART_CHG_FEATURE_MAX_NUM
		...
		bm_debug(
			"%s type:%d, en_ret=%d, active_status=%d,func_val=%d, all_func_status=%#X\n",
			__func__, i, gm->smart_chg[i].en_ret,
			gm->smart_chg[i].active_status,
			gm->smart_chg[i].func_val, all_func_status);
	}
			 ...
			 struct mtk_battery_sysfs_field_info *attr, int val)
{
	...
	bm_err("%s get val:%#X, func_type:%lu, en_ret:%d, func_val:%d\n",
	       __func__, val, func_type, en_ret, func_val);

	bit_pos = find_first_bit(&func_type, SMART_CHG_FEATURE_MAX_NUM);

	/*  */
	if (bit_pos == SMART_CHG_FEATURE_MAX_NUM ||
	    find_next_bit(&func_type, SMART_CHG_FEATURE_MAX_NUM, bit_pos + 1) !=
		    SMART_CHG_FEATURE_MAX_NUM) {
		bm_err("%s ERROR: zero or more than one func type!\n",
		       __func__);
		bm_debug("%s find_next_bit = %lu, bit_pos = %d\n", __func__,
			 find_next_bit(&func_type, SMART_CHG_FEATURE_MAX_NUM,
				       bit_pos + 1),
			 bit_pos);
		set_error(gm);
	} else
	...
	gm->smart_chg[SMART_CHG_STATUS_FLAG].active_status =
		(all_func_status & 0xFFFE) >> 1;
...
int battery_psy_init(struct platform_device *pdev)
{
	...
	psy_register_cooler(gm->bs_data.psy);
