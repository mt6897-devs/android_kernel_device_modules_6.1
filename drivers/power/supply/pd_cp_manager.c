...
static const unsigned char *pm_str[] = {
	"PD_PM_STATE_ENTRY", "PD_PM_STATE_INIT_VBUS", "PD_PM_STATE_ENABLE_CP",
	"PD_PM_STATE_TUNE",  "PD_PM_STATE_EXIT",
};
...
static bool pdm_check_psy(struct usbpd_pm *pdpm)
{
	...
	if (!pdpm->usb_psy) {
		...
		pinfo = (struct mtk_charger *)power_supply_get_drvdata(
			pdpm->usb_psy);
	}
...
static bool pdm_evaluate_src_caps(struct usbpd_pm *pdpm)
{
	...
	for (i = 0; i < pdpm->cap.nr; i++) {
		...
		if (pdpm->cap.type[i] == MTK_PD_APDO &&
		    pdpm->cap.max_mv[i] > pdpm->dts_config.max_pdo_vbus) {
			pdpm->cap.max_mv[i] = pdpm->dts_config.max_pdo_vbus;
			pdpm->cap.maxwatt[i] =
				pdpm->cap.max_mv[i] * pdpm->cap.ma[i];
		}

		if (pdpm->cap.type[i] == MTK_PD_APDO &&
		    pdpm->cap.min_mv[i] >= 5000) {
			pdpm->pd_nobypass = 1;
			pdm_info(
				"dump PDO min_mv=%d, max_mv=%d, ibus=%d, type=%d\n",
				pdpm->cap.min_mv[i], pdpm->cap.max_mv[i],
				pdpm->cap.ma[i], pdpm->cap.type[i]);
		} else {
			pdpm->pd_nobypass = 0;
			pdm_info(
				"dump PDO min_mv=%d, max_mv=%d, ibus=%d, type=%d\n",
		...
		if (pdpm->cap.type[i] != MTK_PD_APDO ||
		    pdpm->cap.max_mv[i] < pdpm->dts_config.min_pdo_vbus ||
		    pdpm->cap.max_mv[i] > pdpm->dts_config.max_pdo_vbus)
			continue;

		if (pdpm->cap.max_mv[i] > MAX_VBUS_67W &&
		    pdpm->cap.ma[i] > MIN_IBUS_67W) {
			...
			if ((pdpm->supported_4_1) &&
			    (pdpm->apdo_max_ibus > pdpm->cap.ma[i])) {
				pdm_info(
					"supported_4_1 = %d, apdo_max_ibus = %d, cap.ma[i] = %d\n",
					pdpm->supported_4_1,
					pdpm->apdo_max_ibus, pdpm->cap.ma[i]);
				legal_pdo = true;
				break;
	...
	if (legal_pdo) {
		...
		for (i = 0; i <= pdpm->cap.nr - 1; i++) {
			if (pdpm->cap.type[i] == MTK_PD_APDO ||
			    !is_between(pdpm->dts_config.min_pdo_vbus,
					pdpm->dts_config.max_pdo_vbus,
					pdpm->cap.max_mv[i]))
				continue;
...
static bool pdm_taper_charge(struct usbpd_pm *pdpm)
{
	...
	if (pdpm->bypass_enable)
		...
		if (is_between(pdpm->dts_config.medium_tbat,
			       pdpm->dts_config.high_tbat, pdpm->tbat))
...
#ifdef CONFIG_TARGET_PRODUCT_PEARL
	cv_vbat = (pdpm->cycle_count > 100) ?
			  pdpm->dts_config.cv_vbat_ffc_large_cycle :
			  pdpm->dts_config.cv_vbat_ffc;
	cv_vbat = pdpm->ffc_enable ? cv_vbat : pdpm->dts_config.cv_vbat;
#else
	cv_vbat = pdpm->ffc_enable ? pdpm->dts_config.cv_vbat_ffc :
				     pdpm->dts_config.cv_vbat;
#endif
...
static void pdm_update_status(struct usbpd_pm *pdpm)
{
	...
	charger_dev_is_bypass_enabled(pdpm->master_dev,
				      &pdpm->master_cp_bypass);
	...
	charger_dev_cp_get_bypass_support(pdpm->master_dev,
					  &pdpm->bypass_support);

	power_supply_get_property(pdpm->bms_psy, POWER_SUPPLY_PROP_CURRENT_NOW,
				  &val);
	pdpm->ibat = val.intval / 1000;

	power_supply_get_property(pdpm->bms_psy, POWER_SUPPLY_PROP_VOLTAGE_NOW,
				  &val);
	...
	power_supply_get_property(pdpm->bms_psy, POWER_SUPPLY_PROP_CAPACITY,
				  &val);
	pdpm->soc = val.intval;

	power_supply_get_property(pdpm->bms_psy, POWER_SUPPLY_PROP_CYCLE_COUNT,
				  &val);
	...
	pdpm->step_chg_fcc =
		get_client_vote(pdpm->fcc_votable, STEP_CHARGE_VOTER);
	pdpm->thermal_limit_fcc =
		min(get_client_vote(pdpm->fcc_votable, THERMAL_VOTER),
		    pdpm->dts_config.max_fcc);
	pdpm->target_fcc = min(get_effective_result(pdpm->fcc_votable),
			       pdpm->dts_config.max_fcc);
	if (!pdpm->pd_verifed && pdpm->target_fcc > 5800)
		pdpm->target_fcc = 5800;

	pdm_info(
		"mt6375 vbus= %d, master_vbus=%d, master_ibus=%d, master_vbatt=%d, slave_ibus=%d, total_ibus=%d, master_enable=%d, slave_enable=%d, cycle_count=%d\n",
		pdpm->mt6375_vbus, pdpm->master_cp_vbus, pdpm->master_cp_ibus,
		pdpm->master_cp_vbatt, pdpm->slave_cp_ibus, pdpm->total_ibus,
		pdpm->master_cp_enable, pdpm->slave_cp_enable,
		pdpm->cycle_count);
	pdm_info(
		"soc=%d, tbat = %d, vbatt=%d, ibatt=%d, jeita_index=%d, vbatt_step=%d, ibatt_step=%d, vbus_step=%d, ibus_step=%d, final_step=%d, target_fcc=%d, sw_cv=%d, charge_full=%d, input_suspend=%d, typec_burn=%d, i2c_error_count=%d, bms_slave_connect_error=%d\n",
		pdpm->soc, pdpm->tbat, pdpm->vbat, pdpm->ibat,
		pdpm->jeita_chg_index, pdpm->vbat_step, pdpm->ibat_step,
		pdpm->vbus_step, pdpm->ibus_step, pdpm->final_step,
		pdpm->target_fcc, pdpm->sw_cv, pdpm->charge_full,
		pdpm->input_suspend, pdpm->typec_burn,
		pdpm->bms_i2c_error_count, pdpm->bms_slave_connect_error);
	pdm_info(
		"master_cp_bypass=%d, slave_cp_bypass=%d, bypass_enable=%d, bypass_support=%d, thermal_limit_fcc=%d\n",
		pdpm->master_cp_bypass, pdpm->slave_cp_bypass,
		pdpm->bypass_enable, pdpm->bypass_support,
		pdpm->thermal_limit_fcc);
}
...
static void pdm_bypass_check(struct usbpd_pm *pdpm)
{
	...
	if (pdpm->cp_work_mode == SC8561_FORWARD_4_1_CHARGER_MODE)
		...
		res = 1;
	if (pdpm->bypass_support && pdpm->apdo_max_watt > MAX_WATT_33W &&
	    pdpm->soc < 90 && pdpm->soc > 5 &&
	    ((apdo_max_watt_temp != 67 && pdpm->soc >= 25) ||
	     (apdo_max_watt_temp == 67))) {
		if (pdpm->bypass_enable) {
	...
	if (pdpm->switch_mode) {
		...
		if ((!pdpm->pd_nobypass) &&
		    (pdpm->last_time.tv_sec == 0 ||
		     time.tv_sec - pdpm->last_time.tv_sec >= 15)) {
...
#endif
	{
		...
		pdpm->target_fcc = pdpm->target_fcc <= apdo_max_ibus_temp ?
					   pdpm->target_fcc :
					   apdo_max_ibus_temp;
	}
	pdm_info("%s apdo_max_ibus=%d target_fcc=%d apdo_max_ibus_temp=%d\n",
		 __func__, pdpm->apdo_max_ibus, pdpm->target_fcc,
		 apdo_max_ibus_temp);
	pdpm->vbus_low_gap =
		pdpm->bypass_enable ? vbus_low_gap_bypass : vbus_low_gap_div;
	pdpm->vbus_high_gap =
		pdpm->bypass_enable ? vbus_high_gap_bypass : vbus_high_gap_div;
	...
	if (pdpm->target_fcc <= 3000)
		pdpm->ibus_gap = 100;
#endif
	pdpm->entry_vbus = min(min(((pdpm->vbat * res) + pdpm->vbus_low_gap),
				   pdpm->dts_config.max_vbus),
			       pdpm->apdo_max_vbus);
	pdpm->entry_ibus = min(min(((pdpm->target_fcc / res) + pdpm->ibus_gap),
				   pdpm->dts_config.max_ibus),
			       apdo_max_ibus_temp);
	pdm_info("%s [%d , %d , %d , %d ] [%d, %d, %d ]\n", __func__,
		 pdpm->bypass_enable, pdpm->switch_mode, pdpm->vbus_low_gap,
		 pdpm->vbus_high_gap, pdpm->ibus_gap, pdpm->entry_vbus,
		 pdpm->entry_ibus);
}
...
static int pdm_tune_pdo(struct usbpd_pm *pdpm)
{
	int fv = 0, ibus_limit = 0, vbus_limit = 0, request_voltage = 0,
	    request_current = 0, final_step = 0, ret = 0, res = 0, fv_vote = 0;
#ifdef CONFIG_TARGET_PRODUCT_XAGA
...
#ifdef CONFIG_TARGET_PRODUCT_PEARL
		fv = (pdpm->cycle_count > 100) ?
			     pdpm->dts_config.fv_ffc_large_cycle :
			     pdpm->dts_config.fv_ffc;
...
#else
		fv = pdpm->ffc_enable ? pdpm->dts_config.fv_ffc :
					pdpm->dts_config.fv;
#endif
	...
	if (pdpm->request_voltage >
	    pdpm->master_cp_vbus +
		    pdpm->total_ibus * MAX_CABLE_RESISTANCE / 1000)
	...
	if (pdpm->bypass_enable)
		apdo_max_ibus_temp = pdpm->apdo_max_ibus * 80 / 100;
	pdpm->ibat_step = pdpm->vbat_step = pdpm->ibus_step = pdpm->vbus_step =
		0;
	ibus_limit = min(min((pdpm->target_fcc / res + pdpm->ibus_gap),
			     apdo_max_ibus_temp),
			 pdpm->dts_config.max_ibus);
	...
	pdm_err("%s vbus_limit=%d ibus_limit=%d apdo_max_ibus_temp=%d, ibus_limit_thr=%d",
		__func__, vbus_limit, ibus_limit, apdo_max_ibus_temp,
		ibus_limit_thr);
	/* ibat loop */
	if ((-pdpm->ibat) <
	    (pdpm->target_fcc - pdpm->dts_config.fcc_low_hyst)) {
		if (((pdpm->target_fcc - pdpm->dts_config.fcc_low_hyst) -
		     (-pdpm->ibat)) > LARGE_IBAT_DIFF)
			pdpm->ibat_step = LARGE_STEP;
		else if (((pdpm->target_fcc - pdpm->dts_config.fcc_low_hyst) -
			  (-pdpm->ibat)) > MEDIUM_IBAT_DIFF)
			pdpm->ibat_step = MEDIUM_STEP;
		else
			pdpm->ibat_step = SMALL_STEP;
	} else if ((-pdpm->ibat) >
		   (pdpm->target_fcc + pdpm->dts_config.fcc_high_hyst)) {
		if (((-pdpm->ibat) -
		     (pdpm->target_fcc + pdpm->dts_config.fcc_high_hyst)) >
		    LARGE_IBAT_DIFF)
			pdpm->ibat_step = -LARGE_STEP;
	...
	final_step = min(min(pdpm->ibat_step, pdpm->vbat_step),
			 min(pdpm->ibus_step, pdpm->vbus_step));
	if (pdpm->step_chg_fcc != pdpm->dts_config.max_fcc || pdpm->sw_cv) {
		if ((pdpm->final_step == SMALL_STEP &&
		     final_step == SMALL_STEP) ||
		    (pdpm->final_step == -SMALL_STEP &&
		     final_step == -SMALL_STEP))
			final_step = 0;
	}
	pdpm->final_step = final_step;
	if (pdpm->bypass_enable)
		cut_cap(pdpm->final_step, bypass_final_step,
			abs(bypass_final_step));

#ifdef CONFIG_TARGET_PRODUCT_XAGA
	if (pdpm->ibat > 0 && pdpm->total_ibus > 0 && pd_tune_count < 30) {
		fcc_ibatt_diff = (pdpm->ibat > pdpm->target_fcc) ?
					 (pdpm->ibat - pdpm->target_fcc) :
					 (pdpm->target_fcc - pdpm->ibat);
		if (fcc_ibatt_diff > 500) {
			pdpm->final_step =
				pdpm->final_step * QUICK_RAISE_VOLT_STEP1_DELTA;
			pd_tune_count++;
		} else if (fcc_ibatt_diff > 250) {
			pdpm->final_step =
				pdpm->final_step * QUICK_RAISE_VOLT_STEP2_DELTA;
			pd_tune_count++;
		}
	}
#endif
	...
	if (pdpm->final_step) {
		request_voltage =
			min(pdpm->request_voltage + pdpm->final_step * STEP_MV,
			    vbus_limit);
		request_current = ibus_limit;
		pdm_err("tune PDO vbus=%d, ibus=%d\n", request_voltage,
			request_current);
		ret = adapter_dev_set_cap_xm(pinfo->pd_adapter, MTK_PD_APDO,
...
static int pdm_check_condition(struct usbpd_pm *pdpm)
{
	...
	if (pdpm->pd_soft_reset) {
		...
		pdm_err("PDM_SM_HOLD input_suspend=%d,typec_burn=%d\n",
			pdpm->input_suspend, pdpm->typec_burn);
		return PDM_SM_HOLD;
	} else if (!is_between(MIN_JEITA_CHG_INDEX, MAX_JEITA_CHG_INDEX,
		...
		pdm_err("PDM_SM_HOLD for fcc target_fcc=%d\n",
			pdpm->target_fcc);
		return PDM_SM_HOLD;
	} else if ((pdpm->state == PD_PM_STATE_ENTRY) &&
		   ((pdpm->soc > pdpm->dts_config.high_soc) ||
		    (is_between(100, 200, pdpm->cycle_count) &&
		     (pdpm->soc > 92)) ||
		...
		return PDM_SM_EXIT;
		   ...
		   (pdpm->target_fcc >= 6000) && (pdpm->cp_4_1_mode == 1) &&
...
static bool pdm_handle_sm(struct usbpd_pm *pdpm)
{
	...
	case PD_PM_STATE_ENTRY:
		...
		if (pdpm->sm_status == PDM_SM_EXIT) {
			...
			adapter_dev_set_cap_xm(pinfo->pd_adapter, MTK_PD_APDO,
					       DEFAULT_PDO_VBUS_1S,
					       DEFAULT_PDO_IBUS_1S);
			return true;
			...
			if (pdpm->target_fcc > 6000 && pdpm->cp_4_1_mode) {
				pdpm->cp_work_mode =
					SC8561_FORWARD_4_1_CHARGER_MODE;
				charger_dev_cp_set_mode(
					pdpm->master_dev,
					SC8561_FORWARD_4_1_CHARGER_MODE);
				charger_dev_cp_set_mode(
					pdpm->slave_dev,
					SC8561_FORWARD_4_1_CHARGER_MODE);
				charger_dev_cp_device_init(pdpm->master_dev,
							   CP_FORWARD_4_TO_1);
				charger_dev_cp_device_init(pdpm->slave_dev,
							   CP_FORWARD_4_TO_1);

			} else if (pdpm->bypass_enable) {
				pdpm->cp_work_mode =
					SC8561_FORWARD_1_1_CHARGER_MODE;
				charger_dev_cp_device_init(pdpm->master_dev,
							   CP_FORWARD_1_TO_1);
				charger_dev_cp_device_init(pdpm->slave_dev,
							   CP_FORWARD_1_TO_1);
				charger_dev_enable_powerpath(pdpm->charger_dev,
							     false);
				msleep(200);
				charger_dev_is_powerpath_enabled(
					pdpm->charger_dev, &power_path_en);
				if (power_path_en) {
					charger_dev_enable(pdpm->charger_dev,
							   false);
					charger_dev_enable_powerpath(
						pdpm->charger_dev, false);
					msleep(200);
				}
			} else {
				pdpm->cp_work_mode =
					SC8561_FORWARD_2_1_CHARGER_MODE;
				charger_dev_cp_set_mode(
					pdpm->master_dev,
					SC8561_FORWARD_2_1_CHARGER_MODE);
				charger_dev_cp_set_mode(
	...
	case PD_PM_STATE_INIT_VBUS:
		...
		if (pdpm->tune_vbus_count == 1) {
			if (pdpm->mt6375_vbus > 3600000 &&
			    gpio_is_valid(pdpm->vbus_control_gpio)) {
				gpio_direction_output(pdpm->vbus_control_gpio,
						      1);
...
#endif
			if (pdpm->pd_type == MTK_PD_CONNECT_PE_READY_SNK_APDO)
				adapter_dev_set_cap_xm(pinfo->pd_adapter,
						       MTK_PD_APDO_START,
						       pdpm->request_voltage,
		...
		if (pdpm->tune_vbus_count >= MAX_VBUS_TUNE_COUNT) {
			...
			break;
		} else if (pdpm->adapter_adjust_count >=
			   MAX_ADAPTER_ADJUST_COUNT) {
			pdm_err("failed to request PDO, exit PDM\n");
			pdm_move_sm(pdpm, PD_PM_STATE_EXIT);
			break;
		}

		if (pdpm->master_cp_vbus <=
		    pdpm->vbat * res + pdpm->vbus_low_gap) {
			pdpm->request_voltage += res * STEP_MV;
		} else if (pdpm->master_cp_vbus >=
			   pdpm->vbat * res + pdpm->vbus_high_gap) {
			pdpm->request_voltage -= res * STEP_MV;
		} else {
		...
		if (pdpm->bypass_enable)
			apdo_max_ibus_temp = pdpm->apdo_max_ibus * 80 / 100;
		pdpm->request_current =
			min(min(pdpm->target_fcc / res, apdo_max_ibus_temp),
			    pdpm->dts_config.max_ibus);
		if (pdpm->pd_type == MTK_PD_CONNECT_PE_READY_SNK_APDO)
			ret = adapter_dev_set_cap_xm(pinfo->pd_adapter,
						     MTK_PD_APDO,
						     pdpm->request_voltage,
						     pdpm->request_current);
	...
	case PD_PM_STATE_ENABLE_CP:
		...
		if (pdpm->master_cp_bypass != pdpm->bypass_enable)
			charger_dev_cp_set_mode(pdpm->master_dev,
						pdpm->cp_work_mode);

		if (pdpm->slave_cp_bypass != pdpm->bypass_enable)
			charger_dev_cp_set_mode(pdpm->slave_dev,
						pdpm->cp_work_mode);

		if (pdpm->master_cp_enable && pdpm->slave_cp_enable &&
		    (!pdpm->bypass_enable ||
		     (pdpm->bypass_enable && pdpm->master_cp_bypass &&
		      pdpm->slave_cp_bypass))) {
			pdm_info("success to enable charge pump\n");
			charger_dev_enable_termination(pdpm->charger_dev,
						       false);
	...
	case PD_PM_STATE_EXIT:
		...
		if (!pdpm->input_suspend) {
			...
			if (pdpm->bms_i2c_error_count >= 3)
				charger_dev_set_input_current(pdpm->charger_dev,
							      500000);
			else
				charger_dev_set_input_current(pdpm->charger_dev,
							      3000000);
...
static void pdm_main_sm(struct work_struct *work)
{
	struct usbpd_pm *pdpm =
		container_of(work, struct usbpd_pm, main_sm_work.work);
	...
	if (!pdm_handle_sm(pdpm) && pdpm->pdm_active) {
		...
		schedule_delayed_work(&pdpm->main_sm_work,
...
{
	struct usbpd_pm *pdpm =
		container_of(work, struct usbpd_pm, psy_change_work);
	int ret = 0, val = 0;
	...
	pdm_info(
		"[pd_type pd_verify_process pdm_active suspend_recovery] = [%d %d %d %d]\n",
		pdpm->pd_type, pdpm->pd_verify_done, pdpm->pdm_active,
		pdpm->suspend_recovery);

	if ((!pdpm->pdm_active || pdpm->suspend_recovery) &&
	    (pdpm->pd_type == MTK_PD_CONNECT_PE_READY_SNK ||
	     pdpm->pd_type == MTK_PD_CONNECT_PE_READY_SNK_PD30 ||
	     pdpm->pd_type == MTK_PD_CONNECT_PE_READY_SNK_APDO) &&
	    pdpm->pd_verify_done) {
...
static int usbpdm_psy_notifier_cb(struct notifier_block *nb,
...
static ssize_t pdm_show_log_level(struct device *dev,
...
static ssize_t pdm_store_log_level(struct device *dev,
...
static ssize_t pdm_show_request_vbus(struct device *dev,
				     struct device_attribute *attr, char *buf)
{
...
static ssize_t pdm_show_request_ibus(struct device *dev,
				     struct device_attribute *attr, char *buf)
{
	struct usbpd_pm *pdpm = dev_get_drvdata(dev);
	int ret = 0;
...
static DEVICE_ATTR(log_level, S_IRUGO | S_IWUSR, pdm_show_log_level,
		   pdm_store_log_level);
...
static int pd_policy_parse_dt(struct usbpd_pm *pdpm)
{
...
#ifdef CONFIG_TARGET_PRODUCT_PEARL
	rc = of_property_read_u32(node, "fv_ffc_large_cycle",
				  &pdpm->dts_config.fv_ffc_large_cycle);
	rc = of_property_read_u32(node, "cv_vbat_ffc_large_cycle",
				  &pdpm->dts_config.cv_vbat_ffc_large_cycle);
#endif
	...
	rc = of_property_read_u32(node, "fcc_low_hyst",
				  &pdpm->dts_config.fcc_low_hyst);
	rc = of_property_read_u32(node, "fcc_high_hyst",
				  &pdpm->dts_config.fcc_high_hyst);
	rc = of_property_read_u32(node, "low_tbat", &pdpm->dts_config.low_tbat);
	rc = of_property_read_u32(node, "high_tbat",
				  &pdpm->dts_config.high_tbat);
	rc = of_property_read_u32(node, "high_vbat",
				  &pdpm->dts_config.high_vbat);
	...
	rc = of_property_read_u32(node, "cv_vbat_ffc",
				  &pdpm->dts_config.cv_vbat_ffc);
	rc = of_property_read_u32(node, "cv_ibat", &pdpm->dts_config.cv_ibat);
	rc = of_property_read_u32(node, "cv_ibat_bypass",
				  &pdpm->dts_config.cv_ibat_bypass);

	rc = of_property_read_u32(node, "vbus_low_gap_div", &vbus_low_gap_div);
	rc = of_property_read_u32(node, "vbus_high_gap_div",
				  &vbus_high_gap_div);
	rc = of_property_read_u32(node, "min_pdo_vbus",
				  &pdpm->dts_config.min_pdo_vbus);
	rc = of_property_read_u32(node, "max_pdo_vbus",
				  &pdpm->dts_config.max_pdo_vbus);

	if (of_property_read_u32(node, "cv_ibat_warm",
				 &pdpm->dts_config.cv_ibat_warm) < 0)
		pdpm->dts_config.cv_ibat_warm = pdpm->dts_config.cv_ibat;

	if (of_property_read_u32(node, "medium_tbat",
				 &pdpm->dts_config.medium_tbat) < 0)
	...
	pdpm->vbus_control_gpio =
		of_get_named_gpio(node, "mt6375_control_gpio", 0);
	if (!gpio_is_valid(pdpm->vbus_control_gpio))
...
#endif
	pdm_info(
		"parse config, FV = %d, FV_FFC = %d, FCC = [%d %d %d], MAX_VBUS = %d, MAX_IBUS = %d, CV = [%d %d %d %d], ENTRY = [%d %d %d %d %d], PDO_GAP = [%d %d %d %d]\n",
		pdpm->dts_config.fv, pdpm->dts_config.fv_ffc,
		pdpm->dts_config.max_fcc, pdpm->dts_config.fcc_low_hyst,
		pdpm->dts_config.fcc_high_hyst, pdpm->dts_config.max_vbus,
		pdpm->dts_config.max_ibus, pdpm->dts_config.cv_vbat,
		pdpm->dts_config.cv_vbat_ffc, pdpm->dts_config.cv_ibat,
		pdpm->dts_config.cv_ibat_warm, pdpm->dts_config.low_tbat,
		pdpm->dts_config.high_tbat, pdpm->dts_config.high_vbat,
		pdpm->dts_config.high_soc, pdpm->dts_config.low_fcc,
		vbus_low_gap_div, vbus_high_gap_div,
		pdpm->dts_config.min_pdo_vbus, pdpm->dts_config.max_pdo_vbus);
