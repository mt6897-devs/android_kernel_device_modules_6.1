	...
	if (ret < 0) {
		...
		for (freq_cnt = 1; freq_cnt < freq_num; freq_cnt++) {
			...
			parse_csvfile(temp_buf, firmware->size, tmp_str,
				      &test_params->noise_threshold[freq_cnt],
				      1, 1);
		}
	}
...
static int goodix_cache_noisedata(struct goodix_ts_test *ts_test)
{
	...
	for (freq_cnt = 0; freq_cnt < freq_num; freq_cnt++) {
		...
		for (cnt = 0; cnt < NOISEDATA_TEST_TIMES; cnt++) {
			...
			if (cd->bus->ic_type == IC_TYPE_BERLIN_D) {
				ret = ts_test_read(ts_test, flag_addr,
						   frame_buf,
						   sizeof(frame_buf));
				if (ret < 0)
					return ret;
				...
				cur_ptr += cd->ic_info.misc.fw_attr_len;
				cur_ptr += cd->ic_info.misc.fw_log_len;
				memcpy((u8 *)ts_test->mutual_freq_diff[freq_cnt]
					       .noisedata[cnt]
					       .data,
				       cur_ptr + 8, data_size * 2);
			} else {
				ret = ts_test_read(
					ts_test, data_addr,
					(u8 *)ts_test
						->mutual_freq_diff[freq_cnt]
						.noisedata[cnt]
						.data,
					data_size * sizeof(s16));
				if (ret < 0)
			...
			ts_test->mutual_freq_diff[freq_cnt].noisedata[cnt].size =
				data_size;
			goodix_rotate_abcd2cbad(
				drv_num, sen_num,
				ts_test->mutual_freq_diff[freq_cnt]
					.noisedata[cnt]
					.data);
			for (i = 0; i < data_size; i++)
				ts_test->mutual_freq_diff[freq_cnt]
					.noisedata[cnt]
					.data[i] =
					ABS(ts_test->mutual_freq_diff[freq_cnt]
						    .noisedata[cnt]
						    .data[i]);
		}
	}
...
static int goodix_analysis_noisedata(struct goodix_ts_test *ts_test)
{
	...
	for (freq_cnt = 0; freq_cnt < freq_num; freq_cnt++) {
		for (cnt = 0; cnt < times; cnt++) {
			for (i = 0; i < data_size; i++) {
				val = ts_test->mutual_freq_diff[freq_cnt]
					      .noisedata[cnt]
					      .data[i];
				if (val > ts_test->test_params
						  .noise_threshold[freq_cnt])
					fail_flag = true;
			}
			if (fail_flag)
				err_cnt++;
...
static void goodix_check_key_info_test(struct goodix_ts_test *ts_test)
{
	...
	if (ts_test->test_params.test_items[GTP_CHIP_KEY_INFO_TEST])
		ts_test->test_result[GTP_CHIP_KEY_INFO_TEST] =
			SYS_SOFTWARE_REASON;
	...
	for (i = 0; i < 3; i++) {
		ret = cd->hw_ops->read_flash(cd, chip_pid_addr,
					     ts_test->chip_pid,
	...
	if (memcmp(ts_test->chip_pid, cd->fw_version.patch_pid, 7)) {
		...
		ts_err("chip_pid:%s != patch_pid:%s", ts_test->chip_pid,
	...
	if (cd->bus->ic_type == IC_TYPE_BERLIN_B ||
		...
		for (i = 0; i < 3; i++) {
			ret = cd->hw_ops->read_flash(cd, chip_info_addr,
						     chip_info_buf,
						     sizeof(chip_info_buf));
			if (ret == 0)
				break;
			msleep(100);
		}
		...
		chksum = chip_info_buf[CHIP_KEY_INFO_MAX_LEN - 2] +
			 ((u16)chip_info_buf[CHIP_KEY_INFO_MAX_LEN - 1] << 8);
		...
		if (chip_info_buf[2 + CHIP_UID_MAX_LEN] == FT_TEST_PASS_FLAG) {
			memcpy(ts_test->chip_info.chip_uid, &chip_info_buf[2],
			       CHIP_UID_MAX_LEN); //UID
			ts_test->chip_info.ft_test_result =
				chip_info_buf[2 + CHIP_UID_MAX_LEN];
		} else if (chip_info_buf[2] == FT_TEST_PASS_FLAG) {
			ts_test->chip_info.ft_test_result = chip_info_buf[2];
			memcpy(ts_test->chip_info.chip_uid, &chip_info_buf[3],
			       CHIP_UID_MAX_LEN); //UID
		} else {
			test_result |= 0x02;
			ts_err("chip info test failed");
		}
		ts_test->chip_info.substrate_tech =
			chip_info_buf[3 + CHIP_UID_MAX_LEN];
		ts_test->chip_info.mcu_fab =
			chip_info_buf[4 + CHIP_UID_MAX_LEN];
		ts_test->chip_info.vaild_len =
			chip_info_buf[CHIP_KEY_INFO_MAX_LEN - 3];
...
static void goodix_custom_info_test(struct goodix_ts_test *ts_test)
{
	...
	if (ts_test->test_params.test_items[GTP_CUSTOM_INFO_TEST])
		ts_test->test_result[GTP_CUSTOM_INFO_TEST] =
			SYS_SOFTWARE_REASON;
...
static int goodix_save_limits(struct goodix_ts_test *ts_test)
{
	...
	if (ts_test->test_result[GTP_VERSION_TEST]) {
		bytes +=
			sprintf(&data[bytes], "<Item name=\"Version Test\">\n");
		bytes += sprintf(&data[bytes],
				 "<CurVerDataString>%s</CurVerDataString>\n",
				 ts_test->cur_version);
		bytes += sprintf(&data[bytes], "</Item>\n");
	}
	if (ts_test->test_result[GTP_CHIP_KEY_INFO_TEST]) {
		bytes += sprintf(&data[bytes],
				 "<Item name=\"Check Chip Key Info\">\n");
		if (GTP_TEST_PASS ==
		    ts_test->test_result[GTP_CHIP_KEY_INFO_TEST])
			bytes += sprintf(&data[bytes],
					 "<TestInfo checkPID=\"true\"/>\n");
		else
			bytes += sprintf(&data[bytes],
					 "<TestInfo checkPID=\"false\"/>\n");
		bytes += sprintf(
			&data[bytes],
			"<ChipInfo Pid=\"%s\" HwVer=\"0x%02x\" PackageSite=\"0x%02x\" FtTestResult=\"0x%02x\" SubstrateTechnology=\"0x%02x\" McuFab=\"0x%02x\">\n",
			ts_test->chip_pid, ts_test->chip_info.hw_ver,
			ts_test->chip_info.package_site,
			ts_test->chip_info.ft_test_result,
			ts_test->chip_info.substrate_tech,
			ts_test->chip_info.mcu_fab);
		bytes += sprintf(&data[bytes], "<ChipUID>");
		for (i = 0; i < sizeof(ts_test->chip_info.chip_uid); i++)
			bytes += sprintf(&data[bytes], "%02x",
					 ts_test->chip_info.chip_uid[i]);
		bytes += sprintf(&data[bytes], "</ChipUID>\n");
	...
	if (ts_test->test_result[GTP_NOISE_TEST]) {
		for (i = 0; i < freq_num; i++)
			str_n = sprintf(
				&tmp_str[str_n], "%d,",
				ts_test->test_params.noise_threshold[i]);
		bytes += sprintf(&data[bytes],
		...
		bytes += sprintf(&data[bytes],
				 "<MaxJitterLimit>%s</MaxJitterLimit>\n",
				 tmp_str);
		bytes += sprintf(&data[bytes], "</Item>\n");
		ret = fs_write(data, bytes);
		if (ret < 0) {
...
static int goodix_save_noise_data(struct goodix_ts_test *ts_test)
{
	...
	for (freq_cnt = 0; freq_cnt < freq_num; freq_cnt++) {
		bytes += sprintf(
			&data[bytes], "<MutualFreq>%d</MutualFreq>\n",
			ts_test->ts->ic_info.parm.mutual_freq[freq_cnt]);
		for (i = 0; i < NOISEDATA_TEST_TIMES; i++) {
			goodix_data_cal(ts_test->mutual_freq_diff[freq_cnt]
						.noisedata[i]
						.data,
					len, stat_result);
			bytes += sprintf(
				   ...
				   struct ts_rawdata_info *info)
{
	...
	ts_info("Have bus error:%d, have panel error: %d", have_bus_error,
		have_panel_error);
...
{
	return single_open_size(file, auto_test_result_show, pde_data(inode),
				DEFAULT_SEQ_FILE_SIZE);
}
...
int inspect_module_init(struct goodix_ts_core *core_data)
{
	...
	proc_entry = proc_create_data("goodix_ts/auto_test", 0660, NULL,
				      &auto_test_ops, core_data);
	if (!proc_entry) {
