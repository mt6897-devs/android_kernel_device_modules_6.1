...
int fts_create_apk_debug_channel(struct fts_ts_data *ts_data)
{
	struct ftxxxx_proc *proc = &ts_data->proc;
	/*#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 10, 0))*/
	proc->proc_entry = proc_create_data(PROC_NAME, 0644, NULL,
...
*/
	...
	ts_data->proc_ta.proc_entry = proc_create_data(
		"fts_ta", 0644, NULL, &fts_procta_fops, ts_data);
...
static ssize_t fts_fw_version_read(struct file *filp, char __user *buf,
				   size_t count, loff_t *pos)
{
	...
	cnt += snprintf(
		&tmp[cnt], PROC_BUF_SIZE,
		"xiaomi-touch version for non thp\n"); //for touchTool apk dump
...
static ssize_t fts_lockdown_info_read(struct file *filp, char __user *buf,
				      size_t count, loff_t *pos)
{
	int cnt = 0, ret = 0;
	char tmp[PROC_BUF_SIZE];
...
int fts_create_proc(struct fts_ts_data *ts_data)
{
	...
	proc->tp_fw_version_proc =
		proc_create("tp_fw_version", 0444, NULL, &tp_fw_version_fops);
...
/* fts_rw_reg */
static ssize_t fts_wr_reg_show(struct device *dev,
			       struct device_attribute *attr, char *buf)
{
	int count;
	int i;
...
static ssize_t fts_wr_reg_store(struct device *dev,
...
static ssize_t fts_touch_point_store(struct device *dev,
				     struct device_attribute *attr,
				     const char *buf, size_t count)
{
	return -EPERM;
...
/* fts_touch_size node */
static ssize_t fts_touch_size_show(struct device *dev,
				   struct device_attribute *attr, char *buf)
{
	int count = 0;
	struct fts_ts_data *ts_data = dev_get_drvdata(dev);
...
static ssize_t fts_touch_size_store(struct device *dev,
				    struct device_attribute *attr,
				    const char *buf, size_t count)
{
	int value = 0;
...
/* fts_ta_mode node */
static ssize_t fts_ta_mode_show(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	int count = 0;
	struct fts_ts_data *ts_data = dev_get_drvdata(dev);
...
static ssize_t fts_ta_mode_store(struct device *dev,
				 struct device_attribute *attr, const char *buf,
				 size_t count)
{
	int value = 0;
...
static ssize_t fts_debug_mode_store(struct device *dev,
				    struct device_attribute *attr,
				    const char *buf, size_t count)
{
	int value = 0;
...
static DEVICE_ATTR(fts_lockdown_info, S_IRUGO | S_IWUSR, fts_lockdown_show,
		   NULL);
static DEVICE_ATTR(fts_debug_mode, S_IRUGO | S_IWUSR, NULL,
		   fts_debug_mode_store);
