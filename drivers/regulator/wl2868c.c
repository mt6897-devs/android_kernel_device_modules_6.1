...
static int wl2868c_regulator_is_enabled(struct regulator_dev *rdev)
{
	...
	rc = wl2868c_read(fan_reg->regmap, WL2868C_REG_ENABLE, &reg, 1);
	if (rc < 0) {
...
static int wl2868c_regulator_enable(struct regulator_dev *rdev)
{
	...
	rc = wl2868c_masked_write(fan_reg->regmap, WL2868C_REG_ENABLE,
...
static int wl2868c_regulator_get_voltage(struct regulator_dev *rdev)
{
	...
	rc = wl2868c_read(fan_reg->regmap, LDO_VSET_REG(fan_reg->offset), &vset,
			  1);
	if (rc < 0) {
...
static int wl2868c_regulator_set_voltage(struct regulator_dev *rdev, int min_uv,
				...
				const char *name)
{
	...
	of_property_read_u32(reg_node, "iout_ua", &wl2868c_reg->iout_ua);
	...
	init_data =
		of_get_regulator_init_data(dev, reg_node, &wl2868c_reg->rdesc);
	if (init_data == NULL) {
	...
	wl2868c_reg->rdev =
		devm_regulator_register(dev, &wl2868c_reg->rdesc, &reg_config);
...
static int wl2868c_parse_regulator(struct regmap *regmap, struct device *dev)
{
	...
	for_each_available_child_of_node (dev->of_node, child) {
		wl2868c_reg =
			devm_kzalloc(dev, sizeof(*wl2868c_reg), GFP_KERNEL);
		if (!wl2868c_reg)
			return -ENOMEM;
...
static DEVICE_ATTR(enable, S_IWUSR | S_IRUSR, wl2868c_show_enable,
		   wl2868c_set_enable);
...
static int wl2868c_driver_register(int index, struct regmap *regmap)
{
	...
	if (alloc_chrdev_region(&(wl2868c_dev.dev_no), 0, 1, device_drv_name)) {
	...
	wl2868c_dev.pdevice =
		device_create(pwl2868c_class, NULL, wl2868c_dev.dev_no, NULL,
			      device_drv_name);
	if (wl2868c_dev.pdevice == NULL) {
		wl2868c_debug(
			"[WL2868C] Allocate device_create for kobject failed\n");
		return -ENOMEM;
	}

	wl2868c_dev.pdevice->driver_data = regmap;
	ret = sysfs_create_file(&(wl2868c_dev.pdevice->kobj),
