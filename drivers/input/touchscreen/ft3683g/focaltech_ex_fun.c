...
/*Modify to be compatible with higher versions of kernel*/
static const struct proc_ops fts_procta_fops = {
    .proc_open = fts_ta_open,
    .proc_release = fts_ta_release,
    .proc_read = fts_ta_read,
...
int fts_create_apk_debug_channel(struct fts_ts_data *ts_data)
{
    struct ftxxxx_proc *proc = &ts_data->proc;
/*#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 10, 0))*/
    proc->proc_entry = proc_create_data(PROC_NAME, 0644, NULL, &fts_proc_fops, ts_data);
...
/*#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 10, 0))*/
    ts_data->proc_ta.proc_entry = proc_create_data("fts_ta", 0644, NULL, &fts_procta_fops, ts_data);
								   ...
								   char __user *buf, size_t count, loff_t *pos)
{
	...
	cnt += snprintf(&tmp[cnt], PROC_BUF_SIZE, "xiaomi-touch version for non thp\n"); //for touchTool apk dump
...
int fts_create_proc(struct fts_ts_data *ts_data)
{
	...
	proc->tp_fw_version_proc = proc_create("tp_fw_version", 0444, NULL, &tp_fw_version_fops);
...
static ssize_t fts_boot_mode_store(
    struct device *dev,
    struct device_attribute *attr, const char *buf, size_t count)
{
    struct fts_ts_data *ts_data = dev_get_drvdata(dev);
...
static ssize_t fts_boot_mode_show(
    struct device *dev, struct device_attribute *attr, char *buf)
{
    ssize_t count = 0;
    struct fts_ts_data *ts_data = dev_get_drvdata(dev);
...
/* fts_rw_reg */
static ssize_t fts_wr_reg_show(
    struct device *dev, struct device_attribute *attr, char *buf)
{
    int count;
    int i;
...
static ssize_t fts_wr_reg_store(
    struct device *dev,
    struct device_attribute *attr, const char *buf, size_t count)
{
    struct fts_ts_data *ts_data = dev_get_drvdata(dev);
...
static ssize_t fts_upgrade_bin_show(
    struct device *dev, struct device_attribute *attr, char *buf)
{
    return -EPERM;
}

static ssize_t fts_upgrade_bin_store(
    struct device *dev,
    struct device_attribute *attr, const char *buf, size_t count)
{
...
static ssize_t fts_force_upgrade_show(
    struct device *dev, struct device_attribute *attr, char *buf)
{
    return -EPERM;
}

static ssize_t fts_force_upgrade_store(
    struct device *dev,
    struct device_attribute *attr, const char *buf, size_t count)
{
...
static ssize_t fts_driver_info_show(
    struct device *dev, struct device_attribute *attr, char *buf)
{
    int count = 0;
    struct fts_ts_data *ts_data = dev_get_drvdata(dev);
...
static ssize_t fts_driver_info_store(
    struct device *dev,
    struct device_attribute *attr, const char *buf, size_t count)
{
    return -EPERM;
...
static ssize_t fts_dump_reg_show(
    struct device *dev, struct device_attribute *attr, char *buf)
{
    int count = 0;
    u8 val = 0;
...
static ssize_t fts_dump_reg_store(
    struct device *dev,
    struct device_attribute *attr, const char *buf, size_t count)
{
    return -EPERM;
...
/* fts_dump_reg interface */
static ssize_t fts_touch_point_show(
    struct device *dev, struct device_attribute *attr, char *buf)
{
    int count = 0;
    int i = 0;
...
static ssize_t fts_touch_point_store(
    struct device *dev,
    struct device_attribute *attr, const char *buf, size_t count)
{
    return -EPERM;
...
/* fts_touch_size node */
static ssize_t fts_touch_size_show(
    struct device *dev, struct device_attribute *attr, char *buf)
{
    int count = 0;
    struct fts_ts_data *ts_data = dev_get_drvdata(dev);
...
static ssize_t fts_touch_size_store(
	struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	int value = 0;
...
/* fts_ta_mode node */
static ssize_t fts_ta_mode_show(
    struct device *dev, struct device_attribute *attr, char *buf)
{
    int count = 0;
    struct fts_ts_data *ts_data = dev_get_drvdata(dev);
...
static ssize_t fts_ta_mode_store(
	struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	int value = 0;
...
static ssize_t fts_debug_mode_store(
	struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	int value = 0;
...
static DEVICE_ATTR_RW(fts_wr_reg);
...
static DEVICE_ATTR_RW(fts_touch_point);
static DEVICE_ATTR_RW(fts_log_level);
static DEVICE_ATTR_RW(fts_pen);
static DEVICE_ATTR_RW(fts_touch_size);
static DEVICE_ATTR_RW(fts_ta_mode);
static DEVICE_ATTR(fts_lockdown_info, S_IRUGO | S_IWUSR, fts_lockdown_show, NULL);
static DEVICE_ATTR(fts_debug_mode, S_IRUGO | S_IWUSR, NULL, fts_debug_mode_store);

/* add your attr in here*/
static struct attribute *fts_attributes[] = {
    &dev_attr_fts_fw_version.attr,
    &dev_attr_fts_wr_reg.attr,
...
void fts_dump_switch(struct fts_ts_data *ts_data, int value)
{
	...
	if(ts_data->tpinfo.success_get != 1) {
		fts_get_tp_info(ts_data);
	}

	mutex_lock(&input_dev->mutex);
	FTS_DEBUG("write 0x9E %x", value);
	fts_write_reg(0x9E, value);
	if(value) {
		ts_data->opendump = 1;
		ts_data->touch_size = ts_data->tpinfo.total_len;
	} else {
		ts_data->opendump = 0;
