...
static int goodix_send_flash_cmd(struct goodix_flash_cmd *flash_cmd)
{
	...
	ret = goodix_reg_write_confirm(flash_cmd_reg, clean_buf,
				       sizeof(clean_buf));
	if (ret < 0) {
		ts_err("clean ack, state as 0xA5 failed");
		return -EAGAIN;
	}

	ret = goodix_reg_write(flash_cmd_reg + 2, flash_cmd->buf + 2,
					    ...
					    const char *buf, size_t count)
{
	...
	goodix_get_config_proc(fw_ctrl->core_data);
	ret = goodix_do_fw_update(
		fw_ctrl->core_data->ic_configs[CONFIG_TYPE_NORMAL], mode);
	if (!ret) {
