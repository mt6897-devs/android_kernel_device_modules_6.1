...
#define GOODIX_DEBUG_ADDR 0x10369
...
static int brl_power_on(struct goodix_ts_core *cd, bool on)
{
	...
	if (on) {
		if (iovdd_gpio > 0) {
			ts_info("enable iovdd gpio");
			gpio_direction_output(iovdd_gpio, 1);
			usleep_range(3000, 3100);
		}
		if (avdd_gpio > 0) {
			ts_info("enable avdd gpio");
		...
		gpio_direction_output(reset_gpio, 0);
		if (iovdd_gpio > 0) {
			ts_info("disable iovdd gpio");
			gpio_direction_output(iovdd_gpio, 0);
		}
		if (avdd_gpio > 0) {
			ts_info("disable avdd gpio");
			gpio_direction_output(avdd_gpio, 0);
		}
...
int brl_gesture(struct goodix_ts_core *cd, int gesture_type)
{
	...
	/*Delete all fingerprints. mode 11 does not deliver 0 for protection*/
	...
	/*Added resend gesture*/
	retry = 0;
	while (retry++ < GOODIX_CMD_RETRY) {
		ret = cd->hw_ops->send_cmd(cd, &cmd);
		if (ret) {
			usleep_range(5000, 5100);
			ts_err("failed to send gesture cmd,retry = %d", retry);
		} else {
			break;
		}
	}
...
static int brl_dev_confirm(struct goodix_ts_core *cd)
{
	...
	while (retry--) {
		ret = hw_ops->write(cd, BOOTOPTION_ADDR, tx_buf,
				    sizeof(tx_buf));
		ts_info("device confirm failed, tx_buf:%*ph", 8, tx_buf);
		if (ret < 0)
			return ret;
		ret = hw_ops->read(cd, BOOTOPTION_ADDR, rx_buf, sizeof(rx_buf));
		ts_info("device confirm failed, rx_buf:%*ph", 8, rx_buf);
		if (ret < 0)
			return ret;
...
static int package_data(int addr, int data_len, u8 *data_buf, u8 *output_buf)
{
	...
	head_info->checksum =
		cpu_to_le32(checksum16_u32(output_buf + 4, data_len + 8));
...
resend_cmd:
	...
	for (i = 0; i < retry_count; i++) {
		...
		if (tmp_buf[0] == status) {
			ts_info("get target state 0x%x, retry cnt: %d", status,
				i);
			usleep_range(5000, 5100);
			return 0;
		}
	...
	ts_err("failed get target state 0x%x != 0x%x, retry %d", tmp_buf[0],
	       status, i);
	return -1;
}
			  ...
			  unsigned char *buf, unsigned int len)
{
	...
	head_info.checksum =
		cpu_to_le32(checksum16_u32((u8 *)&head_info.address, 8));
	//write flash_w cmd
	...
	ret = brl_read(cd, buffer_addr, tmp_buf,
	...
	checksum = checksum16_u32(tmp_buf + 4,
				  read_len + sizeof(flash_head_info_t) - 4);
	if (checksum != le32_to_cpup((u32 *)tmp_buf)) {
		ts_err("read back data checksum error 0x%x != 0x%x", checksum,
		       le32_to_cpup((u32 *)tmp_buf));
		ret = -1;
		goto read_end;
	}
...
read_end:
	ret = goodix_flash_cmd(cd, FLASH_CMD_FINISH, FLASH_CMD_STATUS_PASS,
			       FLASH_CMD_RETRY);
	if (ret)
...
static int brl_flash_write(struct goodix_ts_core *cd, unsigned int addr,
			   unsigned char *buf, unsigned int len)
{
	int ret = -1, i;
	u8 *pack_buf = NULL;
	...
	//write flash_w cmd
	ret = goodix_flash_cmd(cd, FLASH_CMD_W_START, FLASH_CMD_STATE_READY,
			       FLASH_CMD_RETRY);
	if (ret) {
		ts_err("failed enter flash write state");
		return ret;
	}
	pack_buf = kzalloc(sizeof(flash_head_info_t) + FLASH_WRITE_MAX_LEN,
			   GFP_KERNEL);
	if (!pack_buf) {
			   ...
			   int retry)
{
	...
	for (i = 0; i < retry; i++) {
		ts_debug("cmd ack before  is %*ph", (int)sizeof(cmd_ack),
			 cmd_ack.buf);
		ret = hw_ops->read(cd, misc->cmd_addr, cmd_ack.buf,
				   sizeof(cmd_ack));
		/*ts_info("cmd ack after  is %*ph", (int)sizeof(cmd_ack),cmd_ack.buf);*/
		if (!ret && cmd_ack.state == target_status) {
			ts_info("status check pass");
			return 0;
		}
		ts_info("cmd buf %*ph", (int)sizeof(cmd_ack), cmd_ack.buf);
		msleep(10);
	}
...
static void print_ic_info(struct goodix_ic_info *ic_info)
{
	...
	ts_info("panel_x:                       %d", misc->panel_x);
	ts_info("panel_y:                       %d", misc->panel_y);
			   ...
			   struct goodix_ic_info *ic_info)
{
	...
	if (!ic_info->misc.cmd_addr || !ic_info->misc.fw_buffer_addr ||
	    ...
	    !ic_info->misc.self_diffdata_addr) {
		ts_err("cmd_addr fw_buf_addr touch_data_addr self_diffdata_addr is null");
		return -EINVAL;
	}
...
static void goodix_parse_pen(struct goodix_pen_data *pen_data, u8 *buf,
			     int touch_num)
{
	unsigned int id = 0;
	u8 cur_key_map = 0;
				...
				u32 pre_buf_len)
{
	...
	static u8 buffer[IRQ_EVENT_HEAD_LEN +
			 BYTES_PER_POINT * GOODIX_MAX_TOUCH + 2 + 8 + 2];
	u8 touch_num = 0;
	int ret = 0;
	u8 point_type = 0;
	...
	ret = hw_ops->read(cd, GOODIX_DEBUG_ADDR, &debug_buffer[0], 12);
	if (ret)
		ts_info("failed get touch_debug_info");
	if (debug_buffer[1] & 0x80) {
		ts_debug(
			"touch_debug_info %*ph", 12,
			debug_buffer); /*目前看debug信息输出太多，后续固件有优化再按需求改成ts_info*/
	}
			     ...
			     struct goodix_ts_event *ts_event)
{
	...
	ts_debug("touch_data_fromIC %*ph", 26, pre_buf);
	...
	if (checksum_cmp(pre_buf, IRQ_EVENT_HEAD_LEN, CHECKSUM_MODE_U8_LE)) {
		...
		if ((pre_buf[4] == 0x55) && (pre_buf[0] == 0) &&
		    (pre_buf[2] == 0x0a) && (pre_buf[3] == 0x01) &&
		    (cd->work_status == TP_GESTURE)) {
			ts_info("warning: fod up checksum err");
			/*walkaround for avoid drop fod up*/
			pre_buf[0] = 0x28;
		} else if ((pre_buf[4] == 0x46) && (pre_buf[0] == 0) &&
			   (pre_buf[2] == 0x0a) && (pre_buf[3] == 0x01) &&
			   (cd->work_status == TP_GESTURE)) {
			ts_info("warning: fod down checksum err");
			/*walkaround for avoid drop fod down*/
			pre_buf[0] = 0x28;
		} else {
			ts_event->retry = 1;
			return -EINVAL;
...
unsigned char frame_buf_data[FRAME_DATA_MAX_LEN];
			      ...
			      struct ts_rawdata_info *info)
{
	...
	ret = brl_read(cd, flag_addr, frame_buf_data, FRAME_DATA_MAX_LEN);
	if (ret < 0) {
		ts_err("read frame data failed");
		goto exit;
	}

	if (checksum_cmp(frame_buf_data, cd->ic_info.misc.frame_data_head_len,
			 CHECKSUM_MODE_U8_LE)) {
...
static int brl_charger_on(struct goodix_ts_core *cd, bool on)
{
	...
	if (cd->hw_ops->send_cmd(cd, &cmd)) {
		...
		mutex_unlock(&cd->edge_data_mutex);
		return -EINVAL;
	}
	mutex_unlock(&cd->edge_data_mutex);
...
#define GOODIX_SEND_CMD_OK 0X19
#define GOODIX_TEMPBUFFE 0x10098
static int brl_send_Edge_data(struct goodix_ts_core *cd)
{
	int ret;
	int len = 130;
	u8 *tmp_buf;
	...
	/*
	if (len > misc->fw_buffer_max_len) {
		ts_err("edge len exceed limit %d > %d",
    ...
    */
	...
	ts_info("enter brl_send_Edge_data");

	ret = hw_ops->write(cd, misc->self_diffdata_addr, cd->edge_data.buf,
			    len);
	if (ret) {
		ts_err("failed write edge data, %d", ret);
		goto err_quit;
	} else {
		ts_debug("success to write edge data");
	}

	ret = hw_ops->read(cd, misc->self_diffdata_addr, tmp_buf, len);
	if (ret) {
		ts_err("failed read back edge data");
		goto err_quit;
	} else {
		ts_debug("success to read back edge data");
	}

	if (memcmp(cd->edge_data.buf, tmp_buf, len)) {
		ts_err("edge data read back compare fail");
		ret = -EINVAL;
		goto err_quit;
	}
	/*根据供应商建议去掉
	...
	ret = cd->hw_ops->send_cmd(cd, &cfg_cmd);
	if (ret){
		ts_err("failed send edge data ready cmd %d", ret);
		}else{
			ts_info("success to send ready cmd is 0x%x",cfg_cmd);
		}
	*/

	memset(cfg_cmd.buf, 0, sizeof(cfg_cmd));
	cfg_cmd.len = 4;
	cfg_cmd.cmd = GOODIX_SEND_CMD_OK;
	ret = cd->hw_ops->send_cmd(cd, &cfg_cmd);
	if (ret) {
		ts_err("failed send edge write end command");
		ret = -EINVAL;
	} /*else{
		ts_info("succes to send end cmd is 0x%x",cfg_cmd);
	}*/

	if (!ret) {
		ts_info("success send edge data");
		msleep(100);
	}
err_quit:
	kfree(tmp_buf);
...
int brl_Edge_suppression(struct goodix_ts_core *cd)
{
	struct goodix_ts_cmd cmd;
	struct goodix_ts_hw_ops *hw_ops = cd->hw_ops;
	int ret;
	...
	cmd.data[1] = 0x80; /*按照供应商建议a0改成80*/
	/*02:Portrait;60:Horizontal_USB_right;0xa0:Horizontal_USB_left*/
	if (cd->gtp_direction_value == 0) {
		cmd.data[0] = 0x02;
	} else if (cd->gtp_direction_value == 1) {
		cmd.data[0] = 0x60;
	} else if (cd->gtp_direction_value == 3) {
		cmd.data[0] = 0xa0;
	}
	ts_debug("nongamemode screen direction is %d", cd->gtp_direction_value);
	if (!(hw_ops->send_cmd(cd, &cmd))) {
		ret = wait_cmd_status(cd, CONFIG_CMD_STATUS_PASS, 12);
	} else {
		ts_err("send 0x18 failed");
		mutex_unlock(&cd->edge_data_mutex);
		return 0;
	}
	if (!ret) {
		while (retry++ < GOODIX_CMD_RETRY) {
			ret = brl_send_Edge_data(cd);
			if (!ret)
				break;
		}
	} else {
		ts_err("failed when wait_cmd_status");
	}
	mutex_unlock(&cd->edge_data_mutex);
	return 0;
}
...
int brl_game(struct goodix_ts_core *cd, u8 data0, u8 data1)
{
	...
	if (cd->gamemode_enabled) {
		...
		cmd.data[1] = data1;
		if (cd->hw_ops->send_cmd(cd, &cmd)) {
			ts_err("failed send game cmd, data0 = 0x%x, data1 = 0x%x",
			       data0, data1);
			mutex_unlock(&cd->edge_data_mutex);
			return -EINVAL;
		} else {
			ts_info("game data0:0x%x, data1:0x%x, gamemode_enabled = %d",
				data0, data1, cd->gamemode_enabled);
			ret = wait_cmd_status(cd, CONFIG_CMD_STATUS_PASS, 12);
			if (!ret) {
				while (retry++ < GOODIX_CMD_RETRY) {
					ret = brl_send_Edge_data(cd);
				...
				cmd.data[0] = 1;
				if (cd->hw_ops->send_cmd(cd, &cmd)) {
					ts_err("failed send game idle cmd, data0 = %d",
					       cmd.data[0]);
					mutex_unlock(&cd->edge_data_mutex);
					return -EINVAL;
				} else
					ts_info("success send game idle mode");

			} else {
				ts_err("wait_cmd_status failed");
			}
			mutex_unlock(&cd->edge_data_mutex);
		}

	} else {
		ts_info("enter brl edge normal");
	}
	return 0;
}
#endif

static int brl_get_frame_data(struct goodix_ts_core *cd,
			      struct ts_framedata *info)
{
	...
	if (checksum_cmp(info->buff, cd->ic_info.misc.frame_data_head_len,
			 CHECKSUM_MODE_U8_LE)) {
		ts_err("frame head checksum error");
		ret = -EINVAL;
		goto exit;
	...
	if (checksum_cmp(info->buff, frame_head->cur_frame_len,
			 CHECKSUM_MODE_U16_LE)) {
