...
static bool is_typec_adapter(struct mtk_charger *info)
{
	...
	if (info->pd_type == MTK_PD_CONNECT_TYPEC_ONLY_SNK &&
			...
			info->chr_type != POWER_SUPPLY_TYPE_MAINS)
		return true;
...
/* remove atm enabled set 100ma input current*/
	...
	if (info->real_type == XMUSB350_TYPE_FLOAT) {
		...
		retry:
			...
			info->switch_pd_wa--;
			ret = adapter_dev_get_cap(info->pd_adapter, MTK_PD,
						  &cap);
			if (!ret) {
				for (i = 0; i < cap.nr; i++) {
		...
		if (info->switch_pd_wa == -1)
			...
			pdata->input_current_limit =
				info->data.pd2_input_current;

		pdata->charging_current_limit = min(
			info->thermal_current * 1000, info->sic_current * 1000);
		chr_err("pd use vote current charging=%d\n",
			info->thermal_current);
		is_basic = true;
		goto done;
	} else if (info->real_type == XMUSB350_TYPE_HVDCP_2 ||
		   info->real_type == XMUSB350_TYPE_HVDCP_3) {
	...
	if (info->chr_type == POWER_SUPPLY_TYPE_USB &&
		...
		if (info->smart_chg[SMART_CHG_OUTDOOR_CHARGE].en_ret) {
			chr_err("smart_chg_outdoor triggger, DCP set input current 1900mA charging\n");
			pdata->input_current_limit =
				info->data.ac_charger_input_current +
				SMART_CHG_OUTDOOR_CHARGE_INC;
			pdata->charging_current_limit =
				info->data.ac_charger_current +
				SMART_CHG_OUTDOOR_CHARGE_INC;
			if (!info->smart_chg[SMART_CHG_OUTDOOR_CHARGE]
				     .active_status) {
				info->smart_chg[SMART_CHG_OUTDOOR_CHARGE]
					.active_status = true;
			}
		} else {
		...
		chr_err("C to C set input current %dmA charging\n",
			adapter_imax);
...
static int do_algorithm(struct mtk_charger *info)
{
	...
	if (pdata->input_current_limit == 0 ||
		...
		if (!(ret == ALG_READY || ret == ALG_RUNNING) &&
			...
			!info->night_charge_enable && !info->smart_chg[SMART_CHG_NAVIGATION].active_status)
