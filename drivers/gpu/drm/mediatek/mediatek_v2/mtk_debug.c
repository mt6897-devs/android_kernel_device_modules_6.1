			...
			bool blocking)
{
	...
	if (is_frame_mode || !use_lpm ||
		(dsi->ext && dsi->ext->params &&
		(dsi->ext->params->prefetch_time == 0))) {
				  ...
				  bool blocking)
{
	...
	if (blocking) {
		...
		if (!cb_data) {
			DDPPR_ERR("%s:cb data creation failed\n", __func__);
			DDP_MUTEX_UNLOCK(&mtk_crtc->lock, __func__, __LINE__);
			DDP_MUTEX_UNLOCK(&private->commit.lock, __func__,
					 __LINE__);
			CRTC_MMP_EVENT_END(index, ddic_send_cmd, 0, 4);
			return -EINVAL;
		}
	...
	DDP_MUTEX_UNLOCK(&mtk_crtc->lock, __func__, __LINE__);
	DDP_MUTEX_UNLOCK(&private->commit.lock, __func__, __LINE__);
	CRTC_MMP_EVENT_END(index, ddic_send_cmd, (unsigned long)crtc, blocking);
