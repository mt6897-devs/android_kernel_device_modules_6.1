...
void aw86927_haptic_set_rtp_aei(struct aw86927 *aw86927, bool flag)
{
	aw_dbg("%s enter!\n", __func__);
	if (flag) {
...
int aw86927_write_rtp_data(struct aw86927 *aw86927,
...
void aw86927_interrupt_clear(struct aw86927 *aw86927)
{
	unsigned char reg_val = 0;
...
void aw86927_haptic_upload_lra(struct aw86927 *aw86927,
				      unsigned int flag)
{
	unsigned char reg_val;
	...
	case AW86927_OSC_CALI:
		aw_dbg("%s write lra_calib_data to trim_lra = 0x%02X\n",
...
int aw86927_haptic_set_bst_vol(struct aw86927 *aw86927,
...
void aw86927_haptic_set_gain(struct aw86927 *aw86927, unsigned char gain)
{
	...
	if (aw86927->ram_vbat_comp == AW86927_RAM_VBAT_COMP_ENABLE) {
		...
		if (aw86927->effect_id == 10 && ret == 0 && temp <= 0){
			aw_dbg("%s comp_gain:%d ---> %d\n",__func__,comp_gain,aw86927->comp_gain_temp);
			aw86927_i2c_write(aw86927, AW86927_REG_PLAYCFG2,(unsigned char)(aw86927->comp_gain_temp));
		}else{
...
int aw86927_haptic_get_glb_state(struct aw86927 *aw86927,
					unsigned char *state)
{
	aw_dbg("%s enter!\n", __func__);
...
void aw86927_haptic_play_go(struct aw86927 *aw86927)
{
	aw_dbg("%s enter!\n", __func__);
...
int aw86927_haptic_play_mode(struct aw86927 *aw86927,
...
int aw86927_haptic_stop(struct aw86927 *aw86927)
{
	int ret = 0;
...
static int aw86927_rtp_osc_calibration(struct aw86927 *aw86927)
{
	...
	while (1) {
		...
		if (osc_int_state & AW86927_BIT_SYSINT_DONEI) {
			...
			aw_dbg("%s osc trim playback done aw86927->rtp_cnt= %d\n",
		...
		if (aw86927->microsecond > OSC_CALIBRATION_T_LENGTH) {
			aw_dbg("%s osc trim time out! aw86927->rtp_cnt %d osc_int_state %02x\n",
	...
	aw86927->microsecond = ktime_to_us(ktime_sub(aw86927->kend,
						aw86927->kstart));
	/*calibration osc */
	aw_dbg("%s aw86927_microsecond: %ld\n", __func__,
		aw86927->microsecond);
	aw_dbg("%s exit\n", __func__);
	return 0;
}
...
static ssize_t aw86927_comp_gain_temp_show(struct device *dev,
						struct device_attribute *attr,
						char *buf)
{
	struct awinic *awinic = dev_get_drvdata(dev);
	...
	len += snprintf(buf + len, PAGE_SIZE - len,
			"comp_gain_temp=%d\n",
			aw86927->comp_gain_temp);
	aw_info("show comp_gain_temp:%d\n",aw86927->comp_gain_temp);
	return len;
}

static ssize_t aw86927_comp_gain_temp_store(struct device *dev,
						 struct device_attribute *attr,
						 const char *buf, size_t count)
{
	struct awinic *awinic = dev_get_drvdata(dev);
	...
	unsigned int val = 0;
	int rc = 0;
	aw86927->comp_gain_temp = 182;

	aw_info("%s enter\n", __func__);
	rc = kstrtouint(buf, 0, &val);
	if (rc < 0)
		return rc;
	aw86927->comp_gain_temp = val;
	aw_info("write comp_gain_temp:%d\n",aw86927->comp_gain_temp);
	return count;
}
...
static DEVICE_ATTR(comp_gain_temp, S_IWUSR | S_IRUGO,
		   aw86927_comp_gain_temp_show,
		   aw86927_comp_gain_temp_store);
		     ...
		     struct device_node *np)
{
	...
	val = of_property_read_u32(np, "aw86927_comp_gain_temp",
				   &aw86927->comp_gain_temp);
	if (val != 0)
		aw_info("%s comp_gain_temp not found\n", __func__);
...
static void aw86927_vibrator_work_routine(struct work_struct *work)
{
	...
	if (aw86927->state) {
		...
		if (aw86927->wk_lock_flag == 1) {
			pm_relax(aw86927->dev);
			aw86927->wk_lock_flag = 0;
		}
	}
	mutex_unlock(&aw86927->lock);
}
...
/*
**
static uint8_t aw86927_rtp_get_fifo_afi(struct aw86927 *aw86927)
{
	uint8_t reg_val = 0;
...
irqreturn_t aw86927_irq(int irq, void *data)
{
    ...
    if(atomic_read(&aw86927->richtap_rtp_mode) == true)
    {
        if(aw86927_get_irq_state(aw86927))
        {
            pr_debug("%s: aw_haptic rtp fifo almost empty\n", __func__);
            /* modify next line second parameter according to ram.base_addr */
            richtap_update_fifo_data(aw86927, (aw86927->ram.base_addr - (aw86927->ram.base_addr >> 2)));  
            //richtap_update_fifo_data(aw_haptic, (aw_haptic->ram.base_addr >> 1));
            while(!aw86927_rtp_get_fifo_afi(aw86927) && atomic_read(&aw86927->richtap_rtp_mode) && (aw86927->curr_buf->status == MMAP_BUF_DATA_VALID))
            {
                richtap_update_fifo_data(aw86927, (aw86927->ram.base_addr >> 2));
            }
        }

        if(aw86927->curr_buf->status == MMAP_BUF_DATA_INVALID)
        {
            aw86927_haptic_set_rtp_aei(aw86927, false);
...
void aw86927_haptics_set_gain_work_routine(struct work_struct *work)
{
	...
	if (aw86927->ram_vbat_comp == AW86927_RAM_VBAT_COMP_ENABLE
		...
		if (aw86927->effect_id == 10 && ret == 0 && temp <= 0){
			//temp_level = 6*(int)comp_level/5;
			aw_dbg("%s comp_level:%d ---> %d\n",__func__,comp_level,aw86927->comp_gain_temp);
			aw86927_i2c_write(aw86927, AW86927_REG_PLAYCFG2,(unsigned char)(aw86927->comp_gain_temp));
		}else{
