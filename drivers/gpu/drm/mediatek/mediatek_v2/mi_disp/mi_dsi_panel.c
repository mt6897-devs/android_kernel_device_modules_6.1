	...
	unsigned int count, unsigned int format)
{
	...
	if (wait_te_send)
		...
		ret = mtk_ddic_dsi_send_cmd(&cmd_msg, block);
...
int mi_dsi_panel_set_disp_param(struct mtk_dsi *dsi, struct disp_feature_ctl *ctl)
{
	...
	if (!dsi_panel_initialized(dsi)){
		if (DISP_FEATURE_DBI == ctl->feature_id
			...
			panel_ext->funcs->set_gray_by_temperature(dsi->panel, ctl->feature_val);
	...
	case DISP_FEATURE_POWERSTATUS:
		...
		mi_cfg->feature_val[DISP_FEATURE_POWERSTATUS] = ctl->feature_val;
		break;
	...
	case DISP_FEATURE_COLORMODE_NOTIFY:
		...
		mi_cfg->feature_val[DISP_FEATURE_COLORMODE_NOTIFY] = ctl->feature_val;
		mi_dsi_panel_set_cur_color_mode(ctl->feature_val);
		break;
	case DISP_FEATURE_DOLBY_STATUS:
		DISP_INFO("dolby_status:%x\n", ctl->feature_val);
		mi_cfg->feature_val[DISP_FEATURE_DOLBY_STATUS] = ctl->feature_val;
		mi_dsi_panel_set_cur_dolby_status(ctl->feature_val);
		break;
	default:
